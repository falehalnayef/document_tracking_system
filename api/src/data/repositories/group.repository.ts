...
class GroupRepository implements IGroupRepository {
  async getGroupsByAttribute(attributes: {
    [key: string]: any;
  }): Promise<object[]> {
  ...
  async checkUserGroupEntity(
    group_id: number,
    user_id: number
  ): Promise<Object> {
    const userGroupEntity = await UserGroupModel.findOne({
  ...
  async removeUserGroupEntity(
    group_id: number,
    user_id: number
  ): Promise<number> {
    const userGroupEntity = await UserGroupModel.destroy({
      where: { group_id, user_id },
    });
  ...
  async getUserGroupEntity(
    ...
    const userGroupEntity = await UserGroupModel.findAll({
      where: { user_id },
      attributes,
    });
  ...
  async getGroupUserEntity(
    group_id: number,
    attributes: string[]
  ): Promise<object> {
    const groupUserEntity = await UserGroupModel.findAll({
      where: { group_id },
      attributes,
    });
