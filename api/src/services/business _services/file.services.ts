...
class FileServices implements IFileService {
  ...
  constructor(
    ...
    groupServices: IGroupService | null,
    expirationTime: number = 3 * 24 * 60 * 60 * 1000
  ) {
  ...
  async index(groupId: number, userId: number): Promise<IFile[]> {
    ...
    const fileIDsData: IFile[] = (await this.fileRepository.getFileGroupEntity(
      groupId,
      ["file_id"]
    )) as IFile[];
  ...
  async checkFileInGroup(groupId: number, fileId: number): Promise<boolean> {
    const userInGroup = await this.fileRepository.checkFileGroupEntity(
      groupId,
      fileId
    );
    return !!userInGroup;
  ...
  async searchForFile(
    ...
    const fileData = await this.fileRepository.getFilesByLike(
      { file_name: fileName },
      { file_id }
    );
  ...
  async deleteFile(
    ...
    const archived = await this.archiveRepository.getArchivedFilesByFileId(
      fileId
    );
  ...
  async createFile(
    ...
    const filePath = await this.fileOperations.save(
      fileData.data!,
      fileData.fileName
    );
    let file;
    ...
    try {
      await db.sequelize.transaction(async (t: Transaction) => {
        for (let file of await files) {
          const check = await this.fileRepository.checkFileGroupEntity(
            groupId,
            file.file_id
          );
          if (!check) {
            throw new StatusError(
              404,
              `File with ID ${file.file_id} not found in the group ${groupId}`
            );
          }
          if (file.checked) {
            throw new StatusError(
              400,
              `File ${file.file_id} is already checked.`
            );
          }
  ...
  async checkOut(userId: number, fileId: number): Promise<IBooking> {
    ...
    const bookedFileEntity = await this.bookingRepository.getActiveBooking(
      userId,
      fileId
    );
  ...
  async getArchivedFiles(
    ...
    const archivedFiles = await this.archiveRepository.getArchivedFilesByFileId(
      fileId
    );
