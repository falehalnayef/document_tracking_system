...
import {
  ...
  IGroupRepository,
  IGroupService,
} from "../../interfaces/business_interfaces/group.interfaces.js";
import {
  IUser,
  IUserService,
} from "../../interfaces/business_interfaces/user.interfaces.js";
...
class GroupServices implements IGroupService {
  ...
  async indexAsPublic(): Promise<IGroup[]> {
    const groupData = await this.groupRepository.getGroupsByAttribute({
      is_public: true,
    });
  ...
  async indexAsAmember(user_id: number): Promise<IGroup[]> {
    ...
    const groupIDsData: Group[] =
      (await this.groupRepository.getUserGroupEntity(user_id, [
        "group_id",
      ])) as Group[];
  ...
  async getGroupUsers(groupId: number, userId: number): Promise<IUser[]> {
    ...
    const usersIDsData: User[] = (await this.groupRepository.getGroupUserEntity(
      groupId,
      ["user_id"]
    )) as User[];
    ...
    const userData = await this.userServices.userRepository.getUsersByAttribute(
      { user_id },
      ["user_name"]
    );
  ...
  async searchForGroup(groupName: string): Promise<IGroup[]> {
    ...
    const groupData = await this.groupRepository.getGroupsByLike(
  ...
  async deleteFile(fileId: number): Promise<number> {
    ...
    const archived =
      await this.fileService.archiveRepository.getArchivedFilesByFileId(fileId);
  ...
  async deleteGroup(groupId: number, userId: number): Promise<number> {
    ...
    const fileIDsData: IFile[] =
      (await this.fileService.fileRepository.getFileGroupEntity(groupId, [
        "file_id",
      ])) as IFile[];
  ...
  async checkUserInGroup(groupId: number, userId: number): Promise<boolean> {
    const userInGroup = await this.groupRepository.checkUserGroupEntity(
      groupId,
      userId
    );
    return !!userInGroup;
  ...
  async removeUserFromGroup(
    ...
    const bookings =
      await this.fileService.bookingRepository.getBookingsByUserId(userId);
