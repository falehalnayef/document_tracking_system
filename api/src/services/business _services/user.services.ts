...
class UserServices implements IUserService {
  ...
  constructor(
    ...
    hashServices: IHashService,
    authTokenServices: IAuthenticationTokenService
  ) {
  ...
  async login(email: string, password: string): Promise<IUser> {
    ...
    const check = await this.hashServices.compare(
      password,
    ...
    const accessToken = await this.authTokenServices.generateToken(
  ...
  async getAllUsers(): Promise<IUser[]> {
    const users = await this.userRepository.getUsersByAttribute({}, [
      "user_name",
    ]);
