...
class FileController {
  ...
  async createFile(
    ...
    try {
      ...
      res
        .status(201)
        .send(successfulResponse("File is uploaded.", file.display));
    } catch (error: any) {
      if (
  ...
  async searchForFiles(
    ...
    try {
      ...
      const files = await this.fileServices.searchForFile(
        fileName,
        groupId,
        userId
      );
  ...
  async getBookingHistory(
    ...
    try {
      ...
      const bookingHistory = await this.fileServices.getBookingHistory(
        userId,
        groupId,
        fileId
      );
      ...
      res
        .status(200)
        .send(successfulResponse("Booking History", bookingHistory));
    } catch (error: any) {
      next(error);
    }
  }
  ...
  async getArchivedFiles(
    ...
    try {
      ...
      const files = await this.fileServices.getArchivedFiles(
        userId,
        groupId,
        fileId
      );
