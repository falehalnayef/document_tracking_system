...
class IndexRouter {
  ...
  constructor() {
    ...
    const fileSer = new FileServices(
      new FileRepository(),
      bookingRepository,
      archiveRepository,
      validator,
    ...
    this.userRouter = new UserRouter(
      new UserController(userServices),
      this.auth
    ).router;
    this.groupRouter = new GroupRouter(
      new GroupController(groupServices)
    ).router;
  ...
  private initializeRoutes(): void {
    this.router.use(
      "/groups",
      this.auth.checkUser.bind(this.auth),
      this.groupRouter.bind(this.groupRouter)
    );
    this.router.use("/users", this.userRouter.bind(this.userRouter));
    this.router.use(
      "/files",
      this.auth.checkUser.bind(this.auth),
      this.fileRouter.bind(this.fileRouter)
    );
  }
}
